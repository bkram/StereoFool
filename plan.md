...
- **Broadcast-console look pass (6 phases).** Orban Optimod silhouette, HIG-compliant, follows system appearance. `UIBroadcastStyle` namespace centralises palette (safeGreen / tightAmber / overRed / pilotBlue / rdsMagenta) and typography (heroReadout / valueReadout / scaleLabel / chipLabel / chipValue); all palette entries `Color.adaptive(light:, dark:)` so both appearances are intentional. `BroadcastStatusBar` pinned under the window chrome gives operators transport + IN L/R / MPX / DEV / GR / SAFE / BUDGET / PILOT / RDS on every screen. Monitoring embeds compact scopes (100pt) and MPX spectrum (140pt) with pop-out arrows. Processing gains an Overview grid (13 stage cards with enabled toggle + parameter summary + chevron nav) as the default landing tab. Levels window replaces horizontal rows with 8 `VerticalMeterStrip` columns. RDS snapshot cards use a dark meter-plate style; HexCodeField reads as a bordered nameplate. `Card` gains `style: .standard / .meter`. `DSPStatusPill` dots get LED halos. No DSP changes.
...
- **107 new tests across 9 suites.** `RDSTextParserTests` (21), `RDSTextOrchestrationTests` (26), `RDSAdvanceTests` (10), `RDSSignalTests` (9, spectral FFT tests on composite output), `RDSBitstreamTests` (16, decodes emitted 104-bit groups and verifies CRCs / PI / group type / segment / AB flag / Nt: advance / PS scroll end-to-end), `PSBankTests` (12), `DSPThroughputTests` (4, real-time-budget regression tests, see below), `EncoderBandwidthTests` (8, FIR stop-band characterisation — see 7.5 entry above). Plus `RDSBitstreamHelpers.swift` as a shared decode-helper file. 126 / 12 suites green.
